...
public:
  ...
  void drop() {
    std::cout << falling_sphere.body.getGravity().y() << std::endl;
    falling_sphere.body.setGravity(btVector3(0,-10,0));
    dynamics.removeRigidBody(&falling_sphere.body);
    dynamics.addRigidBody(&falling_sphere.body);
    std::cout << falling_sphere.body.getGravity().y() << std::endl;
    std::cout << "HELLOOOOOOEOEOEO???";
  }

  void restart() {
    falling_sphere.body.setGravity(btVector3(0, 0, 0));
    falling_sphere.body.setAngularVelocity(btVector3(0, 0, 0));
    falling_sphere.body.setLinearVelocity(btVector3(0, 0, 0));

    positionCubesAndShpere();

    transform.setOrigin(btVector3(0.9, 3.0, 0));
    falling_sphere.body.setCenterOfMassTransform(transform);
  }

  void move(float x, float y) {
    transform.setOrigin(btVector3(x*0.5f, y*0.5f+0.4f, 0));
    falling_sphere.body.setCenterOfMassTransform(transform);
    positionCubesAndShpere();
  }
...
public:
  Application() : context_settings(24),
                  window(sf::VideoMode(1500, 1200), "SFML Example", sf::Style::Default, context_settings) {
  ...
  void start() {
    ...
    float horizontalPos = 0.9;
    float verticalPos = 3.0;
    ...
    while (running) {
      ...
      if (ImGui::SliderFloat("Horizontal ball position", &horizontalPos, 0.0, 10.0)) {
        world.move(horizontalPos, verticalPos);
      }
      if (ImGui::VSliderFloat("Vertical ball position", {20, 100}, &verticalPos, 0.0, 5.0)) {
        world.move(horizontalPos, verticalPos);
      }
      ImGui::End();
